...
import com.navod.etradeadmin.CallBack.EmployeeUpdateCallback;
...
public class EmployeeService {
    ...
    public void addEmployee(Employee employee, EmployeeAddedCallback callback){
        collectionReference.add(employee)
                .addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                    @Override
                    public void onSuccess(DocumentReference documentReference) {
                        Map<String, Object> fields = new HashMap<>();
                        fields.put("employeeId", documentReference.getId());
                        fields.put("status", true);

                        updateEmployee(documentReference.getId(), fields, new EmployeeUpdateCallback() {
                            @Override
                            public void onSuccess(Boolean success) {
                                if(success){
                                    Log.i(TAG, "Employee added and Updated");
                                }else{
                                    Log.i(TAG, "Employee Updating Failed");
                                }
                            }
                        });
                    }
                    ...
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        callback.onFailure("Employee Adding failed: "+e.getMessage());
                    }
                });
    }
